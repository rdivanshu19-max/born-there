...
export const CountryCard = ({ result, onOpen }: Props) => {
  ...
  return (
    <div className={`glass rounded-2xl p-5 border transition-all hover:-translate-y-0.5 ${toneCls} flex flex-col`}>
      <div className="flex items-start justify-between gap-3 mb-4">
        <div className="flex items-center gap-3 min-w-0">
          <span className="text-3xl shrink-0" aria-hidden>{country.flag}</span>
        ...
        <div className={`text-xs px-2 py-1 rounded-full font-medium tabular-nums ${score > 0 ? "bg-success/15 text-success" : score < 0 ? "bg-destructive/15 text-destructive" : "bg-warning/15 text-warning"}`}>
          {score > 0 ? `+${score}` : score}
        </div>
      </div>
      ...
      <ul className="space-y-1.5 text-sm flex-1">
        {metrics.map((m) => (
          <li key={m.key} className="flex items-center justify-between gap-2">
            ...
            <span className={`inline-flex items-center gap-1 text-xs px-2 py-0.5 rounded-md border ${verdictBg[m.verdict]}`}>
              <Icon v={m.verdict} />
              {fmt(m.theirs, m.unit)}
            </span>
          </li>
      ...
      <Button onClick={onOpen} variant="ghost" size="sm" className="mt-4 self-start text-primary hover:text-primary hover:bg-primary/10 px-2">
        Explore this life <ArrowRight className="h-3.5 w-3.5 ml-1" />
      </Button>
    </div>
  );
};
